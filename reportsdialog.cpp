...
void ReportsDialog::generateSalesByDateReport()
{
    ...
    QVector<Sale> sales = m_db.sales().getSalesByDateRange(startDate, endDate);
...
{
    QVector<Sale> sales = m_db.sales().getAllSales();
    QMap<QString, Money> categoryTotals;
    QMap<QString, int> categoryCount;

    for (const Sale &sale : sales) {
        QVector<SaleItem> items = m_db.sales().getSaleItems(sale.id);
        for (const SaleItem &item : items) {
            Product product = m_db.products().getProductById(item.productId);
...
void ReportsDialog::generateSalesByPaymentReport()
{
    ...
    const QVector<PaymentTotal> totals =
        m_db.salesAnalytics().getPaymentTotalsByMethod(startDate, endDate);
...
{
    QVector<QPair<QString, int>> topProducts = m_db.salesAnalytics().getTopSellingProducts(20);
...
void ReportsDialog::generateProfitLossReport()
{
    ...
    const auto rows = m_db.salesAnalytics().getProfitLossByDateRange(start, end);
...
void ReportsDialog::generateStockValuationReport()
{
    ...
    const auto rows = m_db.products().getStockValuation();
